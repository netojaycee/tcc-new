...
export default async function DesktopNav() {
  const result = await getCategoriesAction({ limit: 13 });
  const categories = result.success && result.data ? result.data.slice(5, 13) : navLinks;

// console.log("DesktopNav categories:", categories);
  return (
